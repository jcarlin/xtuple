  ...
  enyo.kind(_.extend({ },
    ...
    */
    print: function (options) {
      ...
      // reportName and customerForm may come from the workspace calling print
      if (options.reportName) {
        model.reportName = options.reportName;
      }
      if (options.customerForm) {
        model.customerForm = options.customerForm;
      }

      /*if (!model.getPrintParameters) {
        return this.doNotify({message: "Error: model missing getPrintParameters method"});
      }
      ...
      XM.Model.prototype.dispatch(options.module, "getPrintParameters", dispParams, {success: success, error: error});

      var doPrint = function (params) {
        if (!options.printQty && model.quantityAttribute) {
          options.printQty = model.getValue(model.quantityAttribute);
        }
        if (printer && printer !== "Browser") {
          var reportPayload = {
            nameSpace: "ORPT",
            type: params.reportName,
            param: JSON.stringify(params.printParameters), //params
            action: "print",
            printer: printer,
            id: params.id,
          ...
          _.each(params.printParameters, function (param) {
            return routeParams += "&param=%@::%@=%@".f(param.name, param.type, param.value);
          });
          var reportUrl = "/generate-report?nameSpace=%@&type=%@".f(
          "ORPT", params.reportName);

          that.openReport(XT.getOrganizationPath() + reportUrl + routeParams);
        }
      };
    },
    /**
